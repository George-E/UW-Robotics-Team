...
void playNote (void) {
  ...
  int bin[8] = {0};
  if(note != 0){
    convertToBin((int)(127.5 + 127.5*sin(2*(PI)*fqcy[note]*0.0000001*noteTimerCount) +0.5),bin); //added the plus 0.5 so that the cast to int will essentially follow traditional rounding standards
     //pin 0 corresponds with the last bit (highest value) of binary and pin 8 with the first bit 
    for(int i = 0; i < 8; i++){ //each element in the array from 0-7 corresponds to a didital output pin on the teensy. If the bit is on, the teensy outputs a voltage on that bit, wich, with the R2R bridge, results in a wave form as the bits count up and down from 255
...
if (noteTimerCount == 10000000) noteTimerCount = 0; //after one second, restarts the timer count
}
